...
3. For each choice, choose a stable key and repository-scoped topic, then use the script's `hold` command with a concise title and reason.
...
6. After the captain decides, run `audit`, record dependent work with normal tasks-axi commands, and block it by the hold identity.
