...
- Pass `--no-ext-diff` on any `git` command whose patch output is read or parsed, because a host `diff.external=` or `GIT_EXTERNAL_DIFF=` viewer would otherwise substitute its own rendering or fail when that tool is absent.
