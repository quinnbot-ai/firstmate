...
- `FLEET_SYNC: <repo>: STUCK: on <state>, N commits behind <base> - needs attention` - the clone is dirty, on a non-default branch, detached with commits no ref points at, or diverged, so the sync left it untouched (never forcing or discarding); it will keep falling behind until you look.
  ...
  `<base>` is the line the clone actually tracks, which is not always the remote named `origin`, and a clean detached HEAD that a tag or another branch still holds is a deliberate park reported as a benign skip that bootstrap does not relay at all.
