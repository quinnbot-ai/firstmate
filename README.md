...
- **Disposable worktrees** - each task runs in a clean [treehouse](https://github.com/kunchenguid/treehouse) git worktree held until teardown, or an Orca-managed worktree when `backend=orca`, so parallel work on one repo never collides.
...
- **Event-driven, zero-token supervision** - a bash watcher sleeps on the fleet and wakes the first mate only when something needs you; identity-checked relay leases and a verified primary turn-end backstop prevent a live watcher from masking a lost return path.
...
- [docs/watcher-continuity.md](docs/watcher-continuity.md) - watcher-cycle continuity, relay and away-mode daemon leases, and bounded restoration evidence.
