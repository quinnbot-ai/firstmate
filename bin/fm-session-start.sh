...
EOF
    if [ "$event_count" -eq 0 ]; then
      ...
      if [ "$overflow" -eq 1 ]; then
        printf 'home ops-inbox: at least %s event file(s); retained newest %s with full paths:\n' \
          "$((event_count + 1))" "$OPS_INBOX_LIMIT"
      else
...
EOF
      if [ "$overflow" -eq 1 ]; then
        printf '(truncated at least %s older event file(s))\n' "$((event_count - shown + 1))"
