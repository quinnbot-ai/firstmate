...
# spawn on codex too, primary config/crew-claude-profile gives nested Claude
# crewmates the same verified credential store, primary config/backlog-backend=manual makes that home
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness crew-claude-profile backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context}"
