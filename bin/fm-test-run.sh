...
family_for_basename() {
  case "$1" in
    ...
    fm-backlog-graph-lint.test.sh|fm-refill.test.sh|fm-session-lock-ancestry.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-backend.sh|bin/fm-backend-hometag-lib.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-watch*|bin/fm-wake*|bin/fm-refill.sh|bin/fm-backlog-graph-lint.py|\
    ...
    bin/fm-brief.sh|bin/fm-ensure-agents-md.sh|bin/fm-crew-state.sh|bin/fm-nomistakes-liveness-lib.sh|\
