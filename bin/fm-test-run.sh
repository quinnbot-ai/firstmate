...
family_for_basename() {
  case "$1" in
    ...
    fm-send-secondmate-marker.test.sh|fm-shared-captain-inheritance.test.sh)
      printf '%s\n' secondmate
      ;;
    fm-bootstrap.test.sh|fm-code-currency.test.sh|fm-fleet-sync.test.sh|\
    fm-gate-refuse.test.sh|fm-gotmp.test.sh|\
    ...
    fm-spawn-brief-script-reference.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-code-currency-lib.sh|bin/fm-update.sh|\
    ...
    bin/fm-composer-lib.sh)
      ...
      printf '%s\n' live-harness-optin
      ;;
    bin/fm-spawn.sh|bin/fm-brief-script-reference-lib.sh|bin/fm-send.sh|bin/fm-harness.sh|\
