...
#   --base <ref>    with --changed or --check-test-count, compare against this
...
# This intentionally mirrors all_repo_tests: direct tests/*.test.sh files only.
# A merge-base or revision lookup failure is a hard error, so the CI guard can
# never treat an unavailable baseline as a passing count.
test_count_at_revision() {
  local revision=$1
  git ls-tree -r --name-only "$revision" -- tests |
    awk -F/ '$1 == "tests" && NF == 2 && $2 ~ /\.test\.sh$/ { count += 1 } END { print count + 0 }'
}

run_test_count_guard() {
  local candidate merge_base base_count candidate_count shortfall deleted
  ...
  if [ "$candidate_count" -lt "$base_count" ]; then
    shortfall=$((base_count - candidate_count))
    log "test-count floor: candidate has $candidate_count tests, merge base has $base_count (shortfall=$shortfall)"
    deleted=$(git diff --name-status "$merge_base" "$candidate" -- tests |
      awk '$1 == "D" { parts = split($2, path, "/") } parts == 2 && path[1] == "tests" && path[2] ~ /\.test\.sh$/ { print $2 }')
    [ -z "$deleted" ] || {
  ...
  printf 'FM_TEST_COUNT_FLOOR ok merge_base=%s base_ref=%s base_count=%s candidate=%s candidate_count=%s\n' \
    "$merge_base" "$BASE_REF" "$base_count" "$candidate" "$candidate_count"
  return 0
}
