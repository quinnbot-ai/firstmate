...
family_for_basename() {
  case "$1" in
    ...
    fm-no-mistakes-reviewer-recovery.test.sh|\
    ...
    fm-afk-inject-e2e.test.sh|fm-afk-return.test.sh)
      printf '%s\n' afk
      ;;
    fm-bearings-snapshot.test.sh|fm-decision-board.test.sh|fm-fleet-snapshot-view.test.sh)
      printf '%s\n' snapshot-bearings
      ;;
