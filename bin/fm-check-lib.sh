...
FM_STANDING_REVIEW_CHECK_MARKER='# fm-standing-review check - generated by fm-standing-review-arm.sh'

fm_standing_review_id_reserved() {
  case "$1" in
    x-watch|tool-updates) return 0 ;;
    *) return 1 ;;
  esac
}

fm_custom_check_lifecycle_lock_path() {
  local state=$1 id=$2
  fm_pr_task_id_valid "$id" || return 1
  [ -d "$state" ] && [ ! -L "$state" ] || return 1
  printf '%s/.%s.check-lifecycle.lock\n' "$state" "$id"
}

fm_standing_review_check_owned() {
  local check=$1
  [ -f "$check" ] && [ ! -L "$check" ] \
    && head -n 2 "$check" | grep -qFx "$FM_STANDING_REVIEW_CHECK_MARKER"
}
