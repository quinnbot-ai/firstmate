...
sub is_directive_prefix {
  ...
  return 1 if $prefix =~ /^$directed_lead\s+$word(?:\s+$link\s+$word)*$/i;
  return 1 if $prefix =~ /^$directed_lead\s+$directive_verb(?:\s+$link\s+$word)*\s+$executable_object(?:\s+$path_modifier)?\s*:?$/i;
