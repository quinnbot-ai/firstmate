...
#   1. a complete identity-matched supervision chain allows immediately;
...
refresh_supervision_health() {
  watcher_healthy=0
  fm_watcher_healthy "$STATE" "$WATCH" "$GRACE" "$FM_HOME" && watcher_healthy=1
  arm_healthy=0
  if [ "$watcher_healthy" -eq 1 ]; then
    fm_arm_lease_healthy "$STATE" "$WATCH" "$FM_WATCHER_HEALTHY_PID" "$FM_HOME" "$GRACE" \
      && arm_healthy=1
  fi
  ...
  daemon_healthy=1
  if [ "$afk" -eq 1 ]; then
    fm_daemon_lease_healthy "$STATE" "$SCRIPT_DIR/fm-supervise-daemon.sh" "$FM_HOME" "$GRACE" \
      || daemon_healthy=0
  fi
  supervision_healthy=0
  if [ "$afk" -eq 1 ] && [ "$watcher_healthy" -eq 1 ] && [ "$daemon_healthy" -eq 1 ]; then
    supervision_healthy=1
  elif [ "$afk" -eq 0 ] \
    && { { [ "$watcher_healthy" -eq 1 ] && [ "$arm_healthy" -eq 1 ]; } \
      || [ "$checkpoint_healthy" -eq 1 ]; }; then
    supervision_healthy=1
  fi
}
...
refresh_supervision_health
if [ "$supervision_healthy" -eq 1 ]; then
  budget_reset
  exit 0
fi

block_stop() {
  local x_mode reason rule
  refresh_supervision_health
  x_mode=0
  ...
  rule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    ...
    if [ "$FM_SUP_IN_FLIGHT" -eq 0 ]; then
      printf '●  X-mode relay polling needs supervision, but no complete supervision chain is healthy.\n'
    elif [ "$watcher_healthy" -ne 1 ]; then
      printf '●  %s task(s) in flight, but no live watcher holds this home lock (last beat: %s).\n' "$FM_SUP_IN_FLIGHT" "$FM_SUP_BEACON_DESC"
    elif [ "$afk" -eq 1 ] && [ "$daemon_healthy" -ne 1 ]; then
      printf '●  %s task(s) in flight, but away mode has no fresh identity-matched sub-supervisor daemon lease.\n' "$FM_SUP_IN_FLIGHT"
    elif [ "$arm_healthy" -ne 1 ] && [ "$checkpoint_healthy" -ne 1 ]; then
      printf '●  %s task(s) in flight, but the healthy watcher has no fresh identity-matched watch-arm relay lease.\n' "$FM_SUP_IN_FLIGHT"
    else
      printf '●  %s task(s) in flight, but no complete supervision chain is healthy.\n' "$FM_SUP_IN_FLIGHT"
    fi
    if [ "$CLAUDE_MODE" -eq 1 ]; then
...
autoarm_owns_recovery() {
  ...
  refresh_supervision_health
  [ "$supervision_healthy" -eq 1 ] && return 0
...
if [ "$COUNT" -gt "$BLOCK_BUDGET" ]; then
  ...
  printf '{"systemMessage":"firstmate turn-end guard: %s with no complete supervision chain and no Stop auto-arm claim; block budget exhausted, allowing this stop. Repair supervision (bin/fm-watch-arm.sh as a Claude Code background task) or investigate why bin/fm-claude-stop-autoarm.sh is not claiming this home."}\n' "$NEED_DESC"
  exit 0
fi
