...
# Memoized-per-process harness-ancestor resolution. The owner fence runs once
# per watcher cycle, so resolve the harness pid and its stable identity once.
...
# Return 0 when this process may supervise the home. Return 1 only when its
# session lock names a live verified harness outside this process's ancestry.
# A missing, malformed, stale, non-harness, or away-mode lock remains passable.
...
fm_session_owner_fence() {  # <state-dir>
  ...
  if [ "$lock_pid" = "$FM_SESSION_SELF_HARNESS_PID" ] && [ -n "$FM_SESSION_SELF_HARNESS_IDENTITY" ]; then
    ...
    [ "$lock_identity" = "$FM_SESSION_SELF_HARNESS_IDENTITY" ] && return 0
  fi
