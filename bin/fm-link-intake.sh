...
#
# Usage:
#   bin/fm-link-intake.sh upsert --url URL --source-type TYPE --title TITLE --summary SUMMARY --terms TERMS --claim TEXT [--claim TEXT ...] [--canonical-url URL] [--retrieved-at YYYY-MM-DD] [--transcript-file PATH | --transcript-unavailable REASON] [--failure REASON | --pending-reason REASON]
#   bin/fm-link-intake.sh capture --url URL --channel session-chat|relay-inbox
...
# It does not issue filesystem sync barriers and makes no power-loss durability claim.
...
die() {
  if [ "$CAPTURE_CONTEXT" = 1 ]; then
    printf 'error: link-intake-unavailable: %s\n' "$*" >&2
  else
    printf 'error: %s\n' "$*" >&2
  fi
  exit 2
...
`--pending-reason` creates a visible pending record while source inspection awaits.
`capture` durably records an uninspected captain link before source inspection.
...
validate_record() {
  ...
  case "$status" in captured|inaccessible|pending) ;; *) die "record status is invalid: $record" ;; esac
...
EOF
  ...
  if [ "$status" = inaccessible ]; then
    ...
    case "$failure" in pending:*) ;; *) die "pending record has no pending reason: $record" ;; esac
  fi
}
...
command_upsert() {
  local original='' canonical='' canonical_input='' source='' title='' summary='' terms='' retrieved='' transcript_file='' transcript_unavailable='' failure='' pending_reason='' argument claim
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --pending-reason) shift; pending_reason=${1:-} ;;
  ...
  if [ -n "$failure" ] && [ -n "$pending_reason" ]; then
    die 'pending-reason and failure cannot be combined'
  fi
  if [ -n "$pending_reason" ]; then
    require_one_line 'pending reason' "$pending_reason"
    status=pending
    [ -n "$title" ] || title='Pending link inspection'
    [ -n "$summary" ] || summary="Awaiting source inspection: $pending_reason"
    [ -n "$claims" ] || claims="Source inspection is pending: $pending_reason"
    [ -n "$terms" ] || { host=$(url_host "$canonical"); terms="pending,$host"; }
    failure="pending: $pending_reason"
  elif [ -n "$failure" ]; then
...
command_capture() {
  ...
  while [ "$#" -gt 0 ]; do
    ...
    [ "$#" -gt 0 ] || die 'missing value for capture option'
    shift
  done
  require_one_line URL "$original"
  case "$channel" in
    session-chat|relay-inbox) ;;
    *) die 'capture channel must be session-chat or relay-inbox' ;;
  esac
  command_upsert --url "$original" --source-type other \
    --title 'Pending link inspection' \
    --summary "A captain link from $channel is awaiting source inspection." \
    --terms "pending,$(url_host "$(canonicalize_url "$original")")" \
    --claim "Source inspection is pending for the $channel capture." \
    --pending-reason "$channel capture awaits source inspection"
}
