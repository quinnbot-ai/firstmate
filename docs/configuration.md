...
FM_NM_LIVENESS_INTERVAL=60   # seconds between per-task no-mistakes reviewer progress observations
FM_NM_STALL_SECS=1200   # unchanged attributed review status and log tail before the stall becomes one diagnostic wake
FM_NM_LIVENESS_TIMEOUT=10   # seconds allowed per no-mistakes query inside one liveness observation
