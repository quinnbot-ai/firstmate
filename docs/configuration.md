...
It also refuses when this launch's exit receipt cannot be armed or its launch command cannot be wrapped, rather than starting an agent whose exit could never be proven ([`docs/zellij-backend.md`](zellij-backend.md) "Agent exit receipts").
