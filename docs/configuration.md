...
`metrics.<metric>.allowSingleSource` is a per-metric exception that must be explicitly true before one trusted source can publish a `single_source_uncorroborated` value; it does not relax the default two-source rule for any other metric.
