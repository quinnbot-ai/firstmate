...
An absent source configuration renders `source configuration absent`, an unreadable helper renders `source unreadable`, malformed helper output renders `source malformed`, and an otherwise valid helper that omits a requested metric renders `source metric missing`.
A `sources` container that is not an array, an entry without a non-empty array of string command words, or an entry whose optional `timeoutMs` is present but not a positive number, renders `source configuration malformed`, and configuration that survives with fewer than two distinct commands renders `independent source count insufficient`; a diagnosed source failure is reported ahead of that count, so one broken helper stays distinguishable from an unconfigured pair.
A daily helper whose own `date` is a valid calendar date other than the ledger date renders `ledger date not covered` rather than malformed, so a backfill can tell an uncovered day from broken helper output.
Readable daily helpers that disagree on the crew roster, on the top-level currency, or on any amount all render `source disagreement refused`.
Stale, incomplete, or disagreeing source data also remains unavailable rather than zero, while quota-axi windows remain provider-reported windowed measurements rather than exact monetary costs.
