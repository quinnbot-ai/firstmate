...
That archive is the authoritative retained record once Done retention pruning drops an item from `data/backlog.md`, and because `tasks-axi` exposes no archive-read command, firstmate reads the configured archive file directly; unsetting or repointing it leaves pruned records unprovable, which [`docs/decision-hold-lifecycle.md`](decision-hold-lifecycle.md) covers for captain decisions.
...
`AGENTS.md` section 4 owns firstmate's candidate-accounting and final-decision procedure; the selector's single-profile output does not replace that review.
Quota-data trouble never blocks dispatch, but malformed profile configuration remains an actionable validation error.
When this home's `data/claude-crewmate/profile/` is present, it also governs whether Claude candidates are scored through that profile or removed from the candidate set entirely; ["Claude crewmate second-account isolation"](#claude-crewmate-second-account-isolation-dataclaude-crewmate) above owns that contract.
...
Each source may declare a non-empty `provenance` identifier when different commands derive from the same underlying feed, and matching provenance refuses publication rather than mistaking repeated reads for corroboration.
Each metric of the four financial lanes may declare `metrics.<metric>.maxAgeSeconds` to override the lane's `maxAgeSeconds` default with a cadence-appropriate freshness window; `fleet_operations` reads no per-metric declarations, so its operational windows stay at the lane default and no configuration can widen them.
`metrics.<metric>.allowSingleSource` is the captain-authorized `kdp.royalties` exception that must be explicitly true before one trusted source can publish a `single_source_uncorroborated` value; the exemption is scoped in code, so declaring it on any other financial metric publishes nothing and reports `single-source exemption unauthorized` rather than relaxing the default two-source rule.
Within a financial lane, a `metrics` container that is not an object, a metric declaration that is not an object, a `maxAgeSeconds` that is not a non-negative finite number, or a non-boolean `allowSingleSource` reports `metric configuration malformed` instead of falling back to a default.
`fleet_operations.daily_sources` optionally supplies the same corroboration, including distinct provenance, for the dated per-crew session-cost and validation-run line.
Any absent, malformed, unreadable, incomplete, stale, single-source, shared-provenance, or disagreeing source renders its figures unavailable under a named reason rather than zero, while quota-axi windows remain provider-reported windowed measurements rather than exact monetary costs.
...
FM_CREW_STATE_RUNS_SCAN_LIMIT=5000  # run rows requested for the complete listing that must prove a branch never started a pipeline; a listing the CLI truncates fails that proof closed
