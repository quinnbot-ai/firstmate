...
Use `bin/fm-backend-doctor.sh` to read the selected backend's readiness, or repeat `--backend <name>` to inspect named backends without changing selection; the script header owns its exact flags and stable output schema.
