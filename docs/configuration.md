...
Each financial metric may declare `metrics.<metric>.maxAgeSeconds` to override the lane's `maxAgeSeconds` default with a cadence-appropriate freshness window.
`metrics.<metric>.allowSingleSource` is the captain-authorized `kdp.royalties` exception that must be explicitly true before one trusted source can publish a `single_source_uncorroborated` value; the exemption is scoped in code, so declaring it on any other lane or metric publishes nothing and reports `single-source exemption unauthorized` rather than relaxing the default two-source rule.
A `metrics` container that is not an object, a metric declaration that is not an object, a `maxAgeSeconds` that is not a non-negative finite number, or a non-boolean `allowSingleSource` reports `metric configuration malformed` instead of falling back to a default.
