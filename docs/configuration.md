...
The committed `docs/examples/unit-economics-ledger.json` is an installable proposal that pairs the two independent read-only modes of `bin/fm-unit-economics-kdp-source.mjs` for the `kdp` lane; installing it as the private `config/unit-economics-ledger.json` is a deliberate captain step, never automatic.
