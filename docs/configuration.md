...
A `sources` container that is not an array, or an entry without a well-formed string command array and positive `timeoutMs`, renders `source configuration malformed`, and configuration that survives with fewer than two distinct commands renders `independent source count insufficient`; a diagnosed source failure is reported ahead of that count, so one broken helper stays distinguishable from an unconfigured pair.
