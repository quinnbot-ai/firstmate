...
The producing PR and X helpers own the fields they append, `bin/fm-status-report.sh` owns generated crewmate and secondmate material-phase reporting, `bin/fm-classify-lib.sh` owns status-event vocabulary, and `bin/fm-crew-state.sh` owns current-state reconciliation.
...
FM_NOMISTAKES_LIVENESS_INTERVAL=60  # seconds between per-task reviewer/fixer progress observations
FM_NOMISTAKES_STALL_SECS=1200  # unchanged attributed active-step status/log seconds before Firstmate gets a diagnostic wake
FM_NOMISTAKES_LIVENESS_TIMEOUT=10  # seconds allowed per no-mistakes liveness observation
