...
The live private configuration at `/Users/nick/ventures/agent-ops/firstmate/config/unit-economics-ledger.json` contains only `maxAgeSeconds` and an empty `lanes` object.
...
```sh
jq . /Users/nick/ventures/agent-ops/firstmate/config/unit-economics-ledger.json
sqlite3 /Users/nick/ventures/growth/weho-prospect/leads.db 'SELECT * FROM budget_ledger ORDER BY day DESC, source;'
stat -f '%N %Sm' -t '%Y-%m-%dT%H:%M:%SZ' /Users/nick/.openclaw/state/trading-bankroll.json /Users/nick/.openclaw/agents/trader/workspace/data/auto-trader-state.json
jq '.payment_summary' /Users/nick/ventures/publishing/kdp-tools/kindle-novels/data/kdp-analytics.json
bash tests/fm-unit-economics-ledger.test.sh
```

The test suite proves that missing configuration, unreadable commands, malformed source output, and unmapped metrics stay unavailable with distinct reasons.
