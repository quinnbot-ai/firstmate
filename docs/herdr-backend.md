...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call; used for the two fixed spawn-time commands (`treehouse get --lease`, the `GOTMPDIR` export) exactly where tmux used one `send-keys ... Enter` call. |
...
| Submit confirmation (idle baseline) | `herdr agent get <pane>` -> `.result.agent.agent_status` after Enter | `fm_backend_herdr_send_text_submit` requires a live agent before typing, then records the pre-Enter status and, when it is idle/done, confirms delivery by polling for `working`/`blocked` across the Enter attempt's confirmation budget. If an otherwise unreadable post-submit target is dead or has no agent, it returns `send-failed` instead of accepting delivery; composer-state reads remain the affirmative-empty pre-injection guard and the post-submit-aware fallback for preexisting submit-active or unreadable live baselines; see "Native agent-state submit confirmation". |
...
| Kill | `herdr pane close <pane>` | Closing a tab's only (root) pane also closes the tab - no separate tab-close call is needed for this adapter's one-pane-per-tab shape. Routine calls are best-effort, while failed-allocation recovery requests a strict close and retains metadata until target absence is confirmed as defined in [architecture.md](architecture.md#runtime-session-backends). Teardown only closes the task's own pane/tab, never the workspace; closing the last tab also removes that workspace, so a home's workspace persists while it has at least one task tab. See "Default workspace lifecycle" above. |
...
`fm_backend_herdr_pane_agent_state` classifies an existing same-labeled tab's pane as one of `dead` (`pane get` -> `pane_not_found` or `workspace_not_found`, i.e. the pane or its whole workspace is gone), `no-agent` (the pane exists but `agent get` -> `agent_not_found` - the restored-plain-shell shape, and also what a future `resume_agents_on_restore = false` herdr config would produce unconditionally), `live` (a real registered `agent_status`, including idle/blocked - never just "working"), or `unknown` (anything unparseable or unexpected).
...
It first requires the recorded pane to contain a live agent, so a dead pane or plain shell is rejected as `send-failed` before any text is typed.
...
If that fallback becomes unreadable after Enter, the adapter rechecks the pane agent state and returns `send-failed` when it is dead or absent, while retaining `unknown` only for a still-live but inconclusive target.
...
## Incident (2026-07-08): submitted Codex sends falsely reported as swallowed

Three `fm-send` calls to Codex crewmates on Herdr exited non-zero after their messages had submitted and the agents were processing them.
The error reported a swallowed Enter even though the composer was empty apart from Codex's dim ghost hint.

**Root cause.** When a target was already `working` or its native status could not be read, `fm_backend_herdr_send_text_submit` correctly avoided treating that old status as proof of a new submission and fell back to the guard-mode structural composer reader.
During Codex's busy redraw, a submitted user message remains in the transcript as a bare `› <text>` row, followed by its busy footer.
