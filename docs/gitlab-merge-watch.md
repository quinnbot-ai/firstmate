...
Both consumers already treat it as optional: `bin/fm-teardown.sh` reads the head from the forge at teardown rather than from metadata and falls back to its provider-agnostic landed-work proofs, and `bin/fm-review-diff.sh` resolves the head from the remote when none is recorded.
