...
| Kill | `cmux close-workspace --workspace <id>`, preceded by a throwaway `new-workspace --window <win> --focus false --id-format uuids` when the target is the only workspace in its window | Routine kill is best-effort, but it first adds a throwaway sibling when cmux would refuse to close a last workspace; failed-allocation recovery requests a strict close and retains metadata until target absence is confirmed as defined in [architecture.md](architecture.md#runtime-session-backends). |
| Recovery / list-live | `fm_backend_cmux_all_workspaces` (cross-window `list-windows` + `workspace list --json --id-format uuids --window <wid>` per window), filter titles starting with this home's `fm-<home-label>-`, then `list-panes` per match for the surface id | Title-based, never trusts a stored workspace uuid blindly - ids do NOT survive an app relaunch (see "Workspace ids do not survive a relaunch" below), so this is the only safe recovery posture. The adapter prints the plain `fm-<id>` label back to callers after stripping the readable home tag and `FM_ROOT` hash. |
...
`fm_backend_cmux_all_workspaces` closes that gap by enumerating every window (`list-windows`) and running `workspace list --window <wid>` per window, then concatenating the results into one cross-window inventory.
`fm_backend_cmux_workspace_id_for_label`, `fm_backend_cmux_list_live`, `fm_backend_cmux_target_ready`'s label recovery, and `fm_backend_cmux_workspace_absent` all route through it, so label lookup, target-ready recovery, list-live, and absence checks see a task workspace parked in any window.
`fm_backend_cmux_window_of_workspace` continues to pass `--window` per window for its own count/detection.
...
- **Windows cannot be closed over the control socket** - owned by "Closing the last workspace in a window" above. Teardown of a last-in-window task workspace therefore leaves that window a fresh default workspace rather than closing it. Label lookup is no longer current-window scoped: `fm_backend_cmux_workspace_id_for_label`, `fm_backend_cmux_list_live`, `fm_backend_cmux_target_ready`, and `fm_backend_cmux_workspace_absent` all enumerate every window through `fm_backend_cmux_all_workspaces`, so a task workspace parked in a non-current window is still found for label-based recovery (see "Closing the last workspace in a window" above).
