...
| `fm-visual-deliverable-check.sh` | Verify that a rendered visual deliverable served over `http(s)` exposes usable media and interactive controls (`visual-deliverable-usability` skill) |
...
| `fm-continuity-pretool-check.sh` | Claude transport for the bounded watcher-status recovery gate (docs/arm-pretool-check.md) |
| `fm-continuity-command-policy.mjs` | Semantic owner of the Claude continuity gate's fleet-command classification |
...
| `fm-treehouse-lease-lib.sh` | Private durable Treehouse lease handoff shared by spawn and teardown                 |
| `fm-git-identity.sh`     | Pin task-worktree Git identities and audit registered project clone identities       |
| `fm-codex-home.py`       | Create, activate, validate, and remove task-private no-MCP Codex crew homes |
| `fm-claude-auth.py`      | Provision and attest the private Claude crew account, then enforce it under a scrubbed environment before verified exec |
...
| `fm-unit-economics-kdp-source.mjs` | Emit one read-only KDP royalty ledger observation from the payment-telemetry artifact or independently from confirmed monthly finance reports |
...
| `fm-pr-capability.sh`    | Safely preflight whether worker-visible GitHub credentials can create a pull request on `origin` |
...
| `fm-watch-arm.sh`        | Verified home-scoped watcher arm relay with identity-checked lease, loud cycle endings, and bounded lifecycle ledger |
| `fm-watch-checkpoint.sh` | Run one bounded foreground watcher checkpoint for Codex-style supervision            |
| `fm-watch.sh`            | Singleton-safe always-on watcher: absorb benign wakes, queue actionable ones, and fail closed on a lost bound relay |
...
| `fm-supervise-daemon.sh` | Presence-gated away-mode sub-supervisor with an identity-checked liveness lease: self-handle routine wakes, escalate batched digests, alert on failed delivery |
...
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend selector, `tasks-axi` compatibility probe, and exact-id reader for the configured Done archive |
| `fm-wake-drain.sh`       | Atomically drain queued watcher wakes, then assert watcher liveness                  |
| `fm-wake-lib.sh`         | Shared durable wake queue, portable locks, and watcher, relay, and daemon identity/health helpers |
...
| `fm-teardown.sh`         | Fail-closed teardown: return landed ship worktrees safely, require completed scout deliverables, retire secondmate homes |
