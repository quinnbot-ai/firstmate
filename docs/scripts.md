...
| `fm-meta-reconcile.sh`   | Reconcile proven-landed legacy task metadata, dry-run by default and mutating only with `--apply` |
| `fm-teardown-landed-lib.sh` | Shared landed-work proof implementation for teardown and legacy metadata reconciliation |
