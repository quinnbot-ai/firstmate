# Routing the no-mistakes PR step through a firstmate credential
...
This page owns the mechanism firstmate uses to put a PR-capable credential in front of that step, why that mechanism was chosen over the alternatives, and what its limits are.
`bin/fm-gh.sh`, `bin/fm-gh-shim.sh`, and `bin/fm-gh-shim-install.sh` own their exact flags and behavior in their own headers.
...
The shim is a mitigation, not a fix.
It intercepts a command the pipeline did not offer to let anyone configure, so it is invisible to no-mistakes and cannot be validated by it.
...
Retire the shim once such a knob ships.
