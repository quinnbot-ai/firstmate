...
| --- | --- | --- | --- |
| Claude | Run | `.claude/settings.json` registers this tier's one unmatched `SessionStart` hook, invoked through `CLAUDE_PROJECT_DIR` with a 180s timeout; the wrapper reads `source` from the hook payload. | Native stdout context injection is supported. |
...
Claude's tracked `SessionStart` list also carries the unrelated Calm presentation registration owned by [`calm.md`](calm.md#claude-code), deliberately kept as its own grouping so neither adapter inherits the other's ordering, timeout, or source routing.
