...
`bin/fm-supervision-lib.sh` also recognizes an armed standing poll (X-mode relay or operational alert inbox watch) as supervision need, but unaccounted primary work still contributes nothing to that predicate.
Without an independent standing-poll need, unaccounted primary work therefore reads as idle rather than suspicious.
