...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, operations-inbox failures the last baseline did not already carry ([configuration.md](configuration.md#operations-inbox-ops-inbox--configops-inbox-cmd) owns the classification, collapse, and escalation rules), stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, busy panes whose startup, context/token, or subagent-wait signal remains unchanged past the configured busy-progress window, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
When several already-tracked stale rechecks - wedge escalations and declared-pause re-surfaces - come due in the same poll, typically after a watcher outage, the watcher surfaces them as one batched `stale-rechecks` wake that retains each lane's full reason instead of spending one watcher cycle per lane.
Each lane's recheck marker and escalation counter commit only after that batch's durable queue record lands, so a batch preempted by a fresh direct wake leaves its lanes expired to re-collect on the next poll rather than silently deferring them a full cadence.
Fresh first-time stale transitions still surface as individual direct wakes.
...
Between the pane check and that status-log fallback, a `no-mistakes` ship whose done event names only an implementation commit and no URL reports `working` with a delivery-pending detail, because an implementation commit is not delivery; `direct-PR` and `local-only` tasks keep their ordinary done, and a run listing that is unavailable or truncated leaves the event unclassified rather than inventing that state.
...
Claude also keeps the narrow PreToolUse continuity gate as residual active-turn coverage: it allows drain, arm recovery, and fail-closed teardown while refusing only other fleet commands when tasks are in flight and no identity-matched live watcher holds the home lock.
...
The turn-end guard remains the final backstop for every verified harness protocol, cooperating with the auto-arm claim in its `--claude` mode.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work is in flight, direct Stop hooks block and passive turn-end hooks force one bounded follow-up unless the current mode's full relay-health predicate is live.
Normal mode requires the identity-matched watcher lock, fresh beacon, and watch-arm relay lease, while away mode requires the watcher plus the sub-supervisor daemon lease; [turnend-guard.md](turnend-guard.md#relay-health-predicate) owns that predicate.
...
That library also owns the one nonterminal `done:` case: a commit-only no-mistakes done whose branch never started a pipeline is still escalated, but framed as delivery pending rather than as a completion, and it stays terminal whenever the current-state read cannot classify it.
...
The daemon also recognizes the watcher's batched `stale-rechecks` wake and dispatches each bracketed lane through its ordinary stale classification, so a paused lane tracks its pause and a transient lane records its wedge marker without escalating the batch as a unit.
...
A meta whose live `window=` record has been rewritten to a `window_detached_*` record, and that resolves no other endpoint target, is accepted as that recorded proof of absence; any still-resolvable target (for example a retained `tmux_window_id=`) is probed as usual.
...
When an attested captain-populated Claude crew profile is ready, Claude ship and scout workers likewise receive a fresh, task-private `CLAUDE_CONFIG_DIR` under `data/claude-crewmate/` instead of the captain's seat account.
The actual home and final exec must resolve to the profile-local account attestation under an ambient-auth-scrubbed environment.
The home is recorded as `claude_crewmate_home=` and is removed only after endpoint cleanup succeeds, while an absent profile leaves the Claude launch unchanged and any authentication, Keychain provisioning, or identity proof failure refuses the spawn.
...
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` compares against the authoritative base for the task's mode - a freshly fetched `origin/<default>` for remote-backed projects, and the local default branch for `local-only` tasks, whose base is never decided by a remote that may be stale, dead, or absent - and, when task meta records `pr=`, always fetches and compares against `refs/pull/<n>/head` by default (recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
