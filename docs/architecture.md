...
The watcher separately fingerprints Codex MCP startup progress from its existing bounded pane capture, so a busy spinner whose timer churn defeats pane-hash staleness emits an actionable `check: busy-zero-progress` wake after a bounded unchanged-stage threshold.
