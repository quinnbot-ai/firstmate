...
The script header owns the exact run-head ancestry rules, and `bin/fm-nm-run-lib.sh` is their single owner, shared with `bin/fm-teardown.sh`'s pre-teardown run abort so both decide custody identically.
A pipeline that owns its gate fixes in an isolated worktree advances a head that is deliberately unreachable from the crew's worktree, so ancestry cannot decide at all; only then may a complete `branch_sync` custody receipt bind the crew's clean HEAD to that run's submitted head, and a receipt that is missing, names another run, disagrees with the reported head, or claims a cleanliness git contradicts is refused rather than trusted.
`bin/fm-watch.sh` additionally keeps one home-private progress receipt per crew, keyed by the attributed run identity, its active review state, and its review-log tail, because an active run-step proves custody rather than progress.
Only a receipt unchanged for the whole configured bound becomes one diagnostic wake and a matching `fm-crew-state` diagnosis; it never aborts, restarts, or otherwise controls the shared pipeline, and resumed review output or a real approval gate clears it again.
