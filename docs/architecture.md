...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Each spawn separately reports the worker's role, scope, task, and home to Herdr as display-only sidebar metadata that carries no authority over any label or identity; [Sidebar legibility](herdr-backend.md#sidebar-legibility) owns the reported vocabulary, the captain-side row configuration, and that boundary.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, PR-discovery fallback, stale-lock recovery procedure, and the current-owner check that refuses a recorded isolated copy a recycled pool slot has since reassigned.
...
Clean default-branch clones fast-forward to the line the default branch tracks, which is the remote git records for it rather than whatever remote is named `origin`, and a clean detached HEAD that holds no unique commits is re-attached to the default branch before the same fast-forward path runs.
Dirty clones, non-default branches, detached HEADs holding commits no ref points at, diverged defaults, and default branches checked out in another worktree are reported as `STUCK:` with their behind count and left untouched.
A clean detached HEAD that some ref still contains holds nothing that can be lost, so it is left untouched too but reported as a benign park naming that ref, which keeps the loud warning meaning unreferenced work.
