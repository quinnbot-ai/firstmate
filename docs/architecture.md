...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past their recheck gap, and heartbeat backstop hits.
A declared wait's recheck gap starts at `FM_PAUSE_RESURFACE_SECS` and widens with how long that hold has stood, up to `FM_PAUSE_RESURFACE_MAX_SECS`, so a long-lived hold costs steadily fewer turns while still being rechecked on a bounded schedule.
Both the bounded absorb path and the immediate-surface path for a pane whose state cannot be confirmed share that one gap, so neither can notify ahead of the other.
