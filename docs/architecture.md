...
On `attached` it stays live across identity-matched successors, and a close with no verified healthy successor becomes one of two typed nonzero results, separated by whether this arm owned the cycle: an unexplained clean close of an owned child is `watcher: FAILED - cycle ended without an actionable reason`, while a followed cycle's close is `watcher: cycle-ended`, because a wake reason reaches only the arm that forked the watcher.
