...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR merge polling or an X-mode mention, Codex MCP-startup zero-progress checks, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The watcher separately fingerprints Codex MCP startup progress - only for tasks whose meta records the codex harness, and only from the footer lines of its existing bounded pane capture - so a busy spinner whose timer churn defeats pane-hash staleness emits an actionable `check: busy-zero-progress` wake after `FM_BUSY_ZERO_PROGRESS_ESCALATE_SECS` without a stage advance while displayed content elsewhere in the pane cannot spoof startup state.
