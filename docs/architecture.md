...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the normal landed-work contract, PR-discovery fallback, and stale-lock recovery procedure, while [`bin/fm-teardown-landed-lib.sh`](../bin/fm-teardown-landed-lib.sh) provides the shared landed-work implementation used by teardown and explicit legacy metadata reconciliation.
