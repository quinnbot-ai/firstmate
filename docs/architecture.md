...
No-change heartbeats are also benign unless the bounded refill probe reports dispatchable ready work while live ship and scout capacity is below `FM_REFILL_MIN_LIVE`, which defaults to one; that condition emits a wake on the existing heartbeat cadence and preserves its doubling backoff rather than adding a per-poll path.
An emitted heartbeat additionally carries the bounded refill evidence from `bin/fm-refill.sh` on its durable queue payload: how many queued backlog items the configured backlog backend reported as dispatchable and how many ship and scout endpoints were still live when it fired.
...
In away mode, the sub-supervisor reads but does not consume durable heartbeat payloads, processes them in queue order, routes the same actionable refill condition through its existing escalation digest even though evidence-free heartbeats remain force-self-handled, and leaves all away-mode authority boundaries unchanged.
...
The record is atomically replaced under `state/.subsuper-refill-escalation`, and `state/.subsuper-heartbeat-cursor` advances atomically after each handled queue observation while the queue lock excludes concurrent drains.
The daemon's existing per-home singleton lock serializes concurrent cycles.
Refill dedupe state advances only after its escalation buffer append succeeds, so a failed append remains eligible for retry.
...
The deeper session-start recovery probe is separate from that busy-state poll: tmux and Herdr have verified process or native-state classifiers, Zellij accepts only an authenticated per-launch exit receipt as recovery-grade dead proof, and Orca and cmux do not support secondmate spawns.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before delegating to the shared exact-candidate merge boundary in `bin/fm-merge-execute.sh`.
The boundary's inventory and exact-SHA contract is owned by [test-inventory-receipts.md](test-inventory-receipts.md); a well-formed GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) is refused explicitly rather than sent to the wrong forge.
