...
A meta whose live `window=` record has been rewritten to a `window_detached_*` record, and that resolves no other endpoint target, is accepted as that recorded proof of absence; any target `fm_backend_target_of_meta` still resolves (an Orca meta's `terminal=`, or a `window=` record that survived alongside the detached one) is probed as usual.
...
The home is recorded as `claude_crewmate_home=` and is removed only after endpoint cleanup succeeds, while an absent profile leaves the Claude launch unchanged and a present profile that cannot authenticate a task-private copy refuses the spawn.
