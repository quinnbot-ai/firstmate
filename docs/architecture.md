...
A terminal `cancelled` record - never `failed` - yields to an external wait the crew declared after that run started, so an endpoint deliberately left agent-free after a supported abort reports `paused` instead of re-escalating as a possible wedge; the script header owns the exact conditions.
