...
The same fallback reports the distinct `stopped-short` state when a no-mistakes ship crew appended `done:` without the PR-checks-green evidence that its one done gate requires, because the work is committed on a branch with no validated PR behind it.
`bin/fm-classify-lib.sh` owns that gate together with the terminal-evidence test the ci-ready reconciliation shares, so scout, local-only, direct-PR, and metadata carrying no delivery mode are never reclassified; `bin/fm-brief.sh` closes the same gap from the other side by generating a no-mistakes contract that has the worker start validation itself and offers no commit-stage `done:` to conflate with the terminal one.
`bin/fm-fleet-snapshot.sh` surfaces a stopped-short crew as a child-state hold rather than as delivered or terminal work.
