...
Ordinary wake: drain, handle all emitted wakes, reconcile open decisions and unread status lines, and run the exact `--ack-through` command printed as `WAKE_ACK_REQUIRED` on its own, taking its printed `WAKE_ACK_OK` receipt rather than an exit status as proof it landed, then repeat that verified wait while supervision is still required.
