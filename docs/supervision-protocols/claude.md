...
7. The durable wake queue preserves actionable events between a rewake and the next Stop-launched arm, while the bounded turn-end guard prevents a blind Stop when recovery did not start.
   The residual continuity PreToolUse gate allows wake drain, manual arm recovery, and fail-closed teardown while refusing only other `bin/fm-*.sh` fleet commands when tasks are in flight and no identity-matched live watcher holds the home lock.
...
8. The turn-end guard (`bin/fm-turnend-guard.sh --claude`) remains the final backstop.
   It allows the stop when the complete supervision chain is healthy, when the auto-arm already owns recovery for this event epoch, or when a fresh rewake is recorded; it re-blocks only when none of those materialize, within a bounded budget.
