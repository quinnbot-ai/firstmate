...
`bin/fm-test-run.sh` is the single owner of behavior-suite selection, portable CI lane composition, optional local `--jobs` for the proven-isolated set only, per-script timing markers, family totals, the coverage guard, the optional JSON timing artifact, and the optional bounded CI failure receipt.
...
The credential-free CI failure-receipt schema, its bounds, and its aggregate live in [`docs/fm-test-failure-receipts.md`](docs/fm-test-failure-receipts.md).
