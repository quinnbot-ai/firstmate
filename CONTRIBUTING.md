...
- Helper scripts in `bin/` are plain bash.
  ...
  Its `fm-skill-trigger-lint.sh` advisory reports the owning `SKILL.md` path and evidence when frontmatter is an instruction dump, lacks trigger wording, or overlaps an adjacent skill.
  Maintainers resolve a warning by shortening the description into a trigger-led routing phrase or by separating genuinely overlapping scopes; the advisory never rewrites metadata or blocks the lint command.
