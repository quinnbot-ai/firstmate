...
- Helper scripts in `bin/` are plain bash.
  ...
  Its default path also prints `bin/fm-skill-trigger-lint.sh`'s advisory findings, which report the owning `SKILL.md` path and evidence when a skill description is malformed, lacks trigger wording, reads as an instruction dump, or overlaps an adjacent skill.
  Resolve a warning by shortening the description into a trigger-led routing phrase or by separating genuinely overlapping scopes; the advisory never rewrites metadata and never changes the lint's exit status.
