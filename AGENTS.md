...
state/               volatile runtime signals; gitignored
  ...
  .subsuper-* .supervise-daemon.*   sub-supervisor internals and daemon lease; never touch
...
5. **Fleet-state digest** - the compact backlog listing owned by `bin/fm-session-start.sh`; an `ANSWERED-OPEN CAPTAIN DECISIONS` section, printed only when a captain hold already records an answer or when that audit failed, whose handling `decision-hold-lifecycle` owns; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; bounded operations-inbox signals; and one cheap alive/dead read of each task's recorded backend endpoint.
...
`docs/configuration.md` owns dispatch-profile and runtime-backend schemas, `bin/fm-dispatch-select.sh` owns the retained selector mechanics and credential-aware candidate filtering, `bin/fm-harness.sh` owns static resolution, and `bin/fm-spawn.sh` owns launch flags and fail-closed validation.
...
Use the retained selector's provider, product, freshness, and credential-isolation mechanics when evaluating the candidates, but do not treat its single-profile output as a substitute for firstmate's complete candidate accounting.
...
Record completion and retain only the configured recent Done history as part of section 8's closeout-and-refill transaction.
...
Reconcile every terminal ordinary task, and when a routine ship is green or locally ready and its metadata records `yolo=on`, select its existing guarded landing owner by metadata mode: use `bin/fm-merge-local.sh` for `local-only` and `bin/fm-pr-merge.sh` for `no-mistakes` or `direct-PR`; `yolo=off`, unresolved approval, ambiguous state, parked work, and security-sensitive work remain captain-gated and untouched.
...
Use `config/supervision-capacity` as the optional per-home configured capacity source, and count every ordinary `state/*.meta` lane as occupied until safe teardown removes it; a present invalid value blocks refill without disturbing existing lanes.
...
Section 8 owns when automatic queue reevaluation and refill happen; this backlog contract owns eligibility through dependencies and time gates.
...
- `visual-deliverable-usability` - load before reporting a captain-facing rendered visual deliverable complete.
