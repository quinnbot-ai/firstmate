...
state/               volatile runtime signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .busy-zero-progress-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
4. `check:` an actionable check fired, usually a per-task merge or X-mode poll; act on it.
   If it is `check: busy-zero-progress`, inspect the pane and `bin/fm-crew-state.sh <id>`, then load `stuck-crewmate-recovery` if Codex is still stuck in MCP startup.
...
The separate Codex MCP-startup progress check still covers Codex secondmates because a busy startup stage is distinct from healthy idle-pane staleness.
