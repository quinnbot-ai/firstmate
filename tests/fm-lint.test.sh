...
# True only when the ShellCheck fm-lint.sh resolves is exactly the pinned
# version, so the lint-running tests below match what CI enforces instead of a
# runner default.
pinned_ready() {
  local tmp probe
...
test_pins_an_explicit_version() {
  ...
  assert_grep 'fm-install-shellcheck.sh" --required-version' "$LINT" "fm-lint.sh must read the installer-owned ShellCheck pin"
  pass "fm-lint.sh pins an explicit ShellCheck version ($REQUIRED)"
}

test_ci_installs_and_logs_the_pinned_version() {
  # CI must install the installer-owned version and log the resolved binary as
  # parity evidence.
