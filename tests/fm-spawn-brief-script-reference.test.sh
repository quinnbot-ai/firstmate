...
test_markdown_heading_directive_reference_refuses() {
  local id=brief-markdown-heading-a44 rec out status expected
  rec=$(make_case markdown-heading-command "$id" \
    '### Run bin/fm-markdown-heading-missing.sh before editing.')
  read_case "$rec"

  out=$(run_spawn "$id")
  status=$?
  [ "$status" -ne 0 ] || fail "spawn succeeded despite an absent Markdown heading helper instruction"
  expected="$POOL_DIR/bin/fm-markdown-heading-missing.sh"
  assert_contains "$out" "$expected" \
    "Markdown heading helper did not resolve against the task worktree"
  assert_absent "$HOME_DIR/state/$id.meta" \
    "Markdown heading helper refusal published metadata"
...
test_unlisted_declarative_directive_reference_refuses() {
  local id=brief-unlisted-declarative-a48 rec out status expected
  rec=$(make_case unlisted-declarative-command "$id" \
    'It is important that you run bin/fm-unlisted-declarative-missing.sh before editing.')
  read_case "$rec"

  out=$(run_spawn "$id")
  status=$?
  [ "$status" -ne 0 ] || fail "spawn succeeded despite an unlisted declarative helper instruction"
  expected="$POOL_DIR/bin/fm-unlisted-declarative-missing.sh"
  assert_contains "$out" "$expected" \
    "unlisted declarative directive did not resolve against the task worktree"
  assert_absent "$HOME_DIR/state/$id.meta" \
    "unlisted declarative helper refusal published metadata"
  pass "a declarative helper directive does not depend on an adjective allowlist"
}
