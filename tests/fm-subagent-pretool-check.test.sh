  ...
  jq -e '
    [.hooks.PreToolUse[] | select(.matcher == "Bash") | .hooks[].command]
      == [
        ...
        "\"$CLAUDE_PROJECT_DIR\"/bin/fm-continuity-pretool-check.sh"
      ]
  ' "$SETTINGS" >/dev/null || fail "Claude Bash PreToolUse must retain the arm-shape, persistent-cd, and continuity seatbelts"
  jq -e '.hooks.Stop[0].hooks[0].command | contains("fm-turnend-guard.sh")' "$SETTINGS" >/dev/null \
    || fail "the Stop turn-end guard changed"
  pass "Claude wires the delegation guard, retains every Bash seatbelt, and preserves the Stop guard"
}
