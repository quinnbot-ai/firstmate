...
test_atomic_generation_switch_is_process_crash_safe() {
  ...
  assert_grep 'does not issue filesystem sync barriers or claim power-loss durability' "$ROOT/docs/link-intake.md" 'operator contract overstates the process-crash guarantee'
...
test_capture_covers_supported_channels_and_fails_loudly() {
  local channel home record fakebin
  for channel in session-chat relay-inbox; do
    home=$(make_home "capture-$channel")
    run_intake "$home" capture --url 'HTTPS://Capture.Example.test/research#fragment' --channel "$channel" >/dev/null \
      || fail "capture failed for $channel"
    record=$(record_for "$home" 'https://capture.example.test/research')
    assert_grep 'Retrieval status: pending' "$record" "capture did not retain a pending record for $channel"
    assert_grep "Failure: pending: $channel capture awaits source inspection" "$record" "capture did not retain the $channel pending reason"
  done

  home=$(make_home capture-unavailable)
...
SH
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" "$INTAKE" capture --url 'https://capture.example.test/unavailable' --channel session-chat > "$home/capture.out" 2> "$home/capture.err"; then
    fail 'capture accepted a link when canonical storage was unavailable'
  fi
  assert_grep 'error: link-intake-unavailable: could not initialize link-intake storage' "$home/capture.err" 'capture storage failure was not typed and visible'
  [ ! -e "$home/data/link-intake/current" ] || fail 'unavailable capture published a current record'
