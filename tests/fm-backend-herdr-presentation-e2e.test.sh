...
assert_focus_is() {  # <expected> <case-name>
  ...
  if [ "$actual" != "$expected" ]; then
    tail -n 20 "$FOCUS_AUDIT_LOG" >&2 || true
...
normalize_meta() {  # <meta>
  sed -E \
    ...
    -e 's|^tasktmp=.*$|tasktmp=<task-temp-path>|' \
    "$1"
}
...
if ! cmp -s "$TMP_ROOT/off.meta.normalized" "$TMP_ROOT/on.meta.normalized"; then
  ...
  fail "metadata changed beyond ephemeral IDs between flag-off and projected paths"
fi
...
PATH="$FAKEBIN:$PATH" bash -c '
  . "$0/bin/backends/herdr.sh"
  fm_backend_herdr_projection_endpoint_matches_journal "$1" "$2" "$3" "$4"
' "$ROOT" "$HERDR_LAB_SESSION" "$PROJECTED_WSID" "$JOURNAL" shape \
  || fail "projected endpoint no longer correlated with its presentation journal"
...
pass "real Herdr lab: Treehouse commands and metadata shape are byte-identical except for ephemeral IDs"
